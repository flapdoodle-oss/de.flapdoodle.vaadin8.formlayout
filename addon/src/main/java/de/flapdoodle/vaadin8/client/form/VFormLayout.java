package de.flapdoodle.vaadin8.client.form;
...
/**
 * Extension of Vaadin VFormLayout
 */
public class VFormLayout extends SimplePanel {
	...
	public class VFormLayoutTable extends FlexTable implements ClickHandler {
		...
		public static final int COLUMN_FULLSIZE_WIDGET = 0;
		...
		public void setRows(List<ComponentConnector> components, HashMap<Connector, Boolean> widgetHasfullWidth) {
			int rowNr = components.size();
			for (int i = 0; i < rowNr; i++) {
				if (widgetHasfullWidth.containsKey(components.get(i))) {
					initFullsizeRow(i, rowNr);
				} else {
					initDefaultRow(i, rowNr);
				}
			}
		...
		private void initFullsizeRow(int row, int max) {
			prepareCell(row, COLUMN_FULLSIZE_WIDGET);
			getCellFormatter().setStyleName(row, COLUMN_FULLSIZE_WIDGET,
					CLASSNAME + "-contentcell");
			getFlexCellFormatter().setColSpan(row, COLUMN_FULLSIZE_WIDGET, 3);
			...
			getRowFormatter().setStyleName(row, rowstyles);
		}

		private void initDefaultRow(int row, int max) {
			prepareCell(row, COLUMN_CAPTION);
			getCellFormatter().setStyleName(row, COLUMN_CAPTION,
					CLASSNAME + "-captioncell");

			prepareCell(row, COLUMN_ERRORFLAG);
			getCellFormatter().setStyleName(row, COLUMN_ERRORFLAG,
					CLASSNAME + "-errorcell");

			prepareCell(row, COLUMN_WIDGET);
			getCellFormatter().setStyleName(row, COLUMN_WIDGET,
					CLASSNAME + "-contentcell");
	...
	public class Caption extends HTML {
		 ...
		 *
		 * @param component
		 *          optional owner of caption. If not set, getOwner will return null
		 */
