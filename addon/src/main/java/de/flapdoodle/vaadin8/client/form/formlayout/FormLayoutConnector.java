package de.flapdoodle.vaadin8.client.form.formlayout;
...
/**
 * Copy of Vaadin FormLayoutConnector
 */
@Connect(de.flapdoodle.vaadin8.FormLayout.class)
public class FormLayoutConnector extends AbstractLayoutConnector
	...
	private final ElementResizeListener resizeListener = event -> {
		...
		if (ownWidth < tableWidth) {
			...
			int fullRowtargetWidth = (int) Math.floor(tableWidth - excessWidth); // ownWidth

			// Target might be negative if captions are wider than the total
			// available width
			targetWidth = Math.max(0, targetWidth);
			fullRowtargetWidth = Math.max(0, fullRowtargetWidth);

			if (oldMaxWidths == null) {
				oldMaxWidths = new HashMap<>();
			}

			HashMap<Connector, Boolean> useFullRow = getState().useFullRow;

			for (ComponentConnector child : getChildComponents()) {
				Element childElement = child.getWidget().getElement();
				int currentTargetWidth = targetWidth;
				if (useFullRow.containsKey(child)) {
					currentTargetWidth = fullRowtargetWidth;
				}
				if (!oldMaxWidths.containsKey(child)) {
					oldMaxWidths.put(child,
							childElement.getPropertyString("maxWidth"));
				}
				childElement.getStyle().setPropertyPx("maxWidth", currentTargetWidth);
				layoutManager.reportOuterWidth(child, currentTargetWidth);
			}
		}
	};
	...
	public void onConnectorHierarchyChange(
		...
		formLayoutTable.setRows(getChildComponents(), getState().useFullRow);
